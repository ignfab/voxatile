...
- `place`: *Placeable* to place at each heightmap voxel.
...
place: default:grass
```
This will put a grass voxel at heightmap altitude. For better rendering, place voxel structure instead of voxel.
...
- `inside`: *Placeable* to place on each inside voxel
- `edge`: *Placeable* to place on each edge voxel
...
A placeable is something that can be placed in voxel world at a given position : a voxel, a structure, a pattern.
...
When a string is given as value for a placeable field, it is interpreted as block type for Minecraft and node type for Minetest. All other parameters are set to default (see Minecraft and Minetest sections bellow). Other future formats may accept or not this shortcut.
...
**NOTE**: This is an unfinished version intended to evolve soon.
...
Places a stack of placeable bellow or over the reference position. Stack is made of several different placeable repeated a given number of times.
