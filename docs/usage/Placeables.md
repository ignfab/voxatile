...
* [Voxels](#voxels)
  * [Minecraft voxel description](#minetest-voxel-description)
  * [Minetest voxel description](#minecraft-voxel-description)
...
Places nothing. This conveniant if material is required but you want to place nothing:
...
Structure can be described as a list of boxes filled with an placeable (could be a voxel, but could be another structure):
...
Boxes are notated as : `[x, y, z]` where `x`, `y` and `z` can be an interval notated `start..end` where `start` and `end` are starting and ending value of the interval, or a single coordinate (`5` is equivalent to `5..5`). All values are integers.


This will give a structure with a 3x3x3 dirt cube, with grass on top and a tree trunk over it.
...
Another way to describe structures uses "ASCII art" blueprints. Note that any way is used to describe structure, structure object is the same at the end.
...
Above structure parameters describes a 2-D checkboard:

![A checkboard made of wood and back and white wool voxels floating in the air](img/structure-checkboard.png)
...
That draws two pawns on the checkboard:

![A checkboard made of wood and back and white wool voxels floating in the air with a white and a red pawn on it](img/structure-mixed.png)
