...
allBuildsDone:
  ...
  after:
    ...
    - buildFloors
```
This task can be used in `after` fields of other task needing buildings to be set up before they start.
...
And any other task that referring to a `schedule` or `sequence` task in its `after` list will start once that `schedule` or `sequence` task have ended (i.e. once all subtasks have ended).
...
Inheritance is performed by intersecting model selections from different levels. This may result in an empty selection if different types or contradictory filters are combined.
