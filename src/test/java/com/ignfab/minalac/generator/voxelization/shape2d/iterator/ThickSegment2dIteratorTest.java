...
public class ThickSegment2dIteratorTest {
    ...
    private static List<Segment2d> segments = List.of(
    ...
    // We won't tests exactly what this iterator does because it could vary without seriously affecting visual effect.
    ...
    public void testConnected() {
        ...
        for (Segment2d segment : segments) {
            boolean[][] result = renderIterator(new ThickSegment2dIterator(segment, 5, segment.normal(), segment.normal().opposite()));

            for (int x = 1; x < limitX - 1; x++)
                for (int y = 1; y < limitY - 1; y++)
                    if (result[x][y] && !(result[x + 1][y] || result[x][y + 1] || result[x - 1][y] || result[x][y - 1]))
                        fail("Segment %s: Expected a connected voxel at (%d, %d).".formatted(segment, x, y));
        }
    }

    // Test we can escape to the border from a given position (x, y) in a given direction (dx, dy)
    ...
    public void testNoHoles() {
        ...
        // As we are on a segment, each non set voxel should be on top/left/bottom/right most position or it's a hole.
                ...
                for (int y = 1; y < limitY - 1; y++)
                    if (!result[x][y])
                        assertTrue(
                            ...
                            || testEscape(result, x, y, 0, -1),
                            "Segment %s: Unexpected hole at (%d, %d).".formatted(segment, x, y));
        }
    }
}
