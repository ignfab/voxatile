...
public class ThickSegment2d5Iterator implements Iterator<Positioned3d> {
    ...
    public ThickSegment2d5Iterator(Segment3d segment, double thickness, Vector2d startBevelDirection, Vector2d endBevelDirection) {
        ...
        factor = segment.length() > 0 ? 1.0 / segment.length() : 1.0;
    ...
    public Positioned3d next() {
        ...
        // A linear Z value is simply added to 2d position (factor = 1 / length2d)
        return position.coords().to3d((int) Math.round(
            segment.start().z() + (segment.end().z() - segment.start().z()) * segment2d.nearestPointIndex(position.coords()) * factor
        ));
    }
}
