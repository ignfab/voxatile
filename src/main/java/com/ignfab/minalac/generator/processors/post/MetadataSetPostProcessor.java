...
public class MetadataSetPostProcessor extends PostProcessor.Generic {
    ...
    private final boolean keepExistingMetadata;
     ...
     * @param keepExistingMetadata whether to abort if it would overwrite existing metadata
     */
    public MetadataSetPostProcessor(String metadata, ModelValue value, boolean abortIfValueIsAbsent, boolean keepExistingMetadata) {
        this.metadata = metadata;
        this.value = value;
        this.abortIfValueIsAbsent = abortIfValueIsAbsent;
        this.keepExistingMetadata = keepExistingMetadata;
    }

    @Override
    public Model process(Model model) {
        if (keepExistingMetadata && model.hasMetadata(metadata))
            return model;
