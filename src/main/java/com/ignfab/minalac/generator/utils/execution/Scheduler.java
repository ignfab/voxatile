...
public class Scheduler implements Schedule {
     ...
     */
    public void run(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException, GenerationFailedException, TaskFailedException {
        tasks.values().forEach(ScheduledTask::reset);
        Future<Void> future = executor.submit(() -> launchTasks());
        try {
