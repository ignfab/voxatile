    ...
    public void render(WorldBBox3d bbox) {
        for (Model model : models) {
            ...
            for (Voxel2d voxel : voxelizer) {
                ...
                } catch (OutOfWorldException ignored) {} // Should never happen. Maybe consider modifying this exception as a RuntimeException?
            }
            for (IndexedVoxel2d voxel : voxelizer.borders()) {
                WorldCoords2d c = voxel.coords();
                try {
                    edge.place(c.x(), c.y(), heightmap.get(c) + 1);
                } catch (OutOfWorldException ignored) {} // Should never happen
            }
        }
    }
}
