...
/**
 * Parameters for a {@link RandomUniformModelValue}.
 */
public class RandomUniformModelValueParams extends ModelValueParams {
    /**
     * The lower bound (inclusive) of the value (required).
     */
    @JsonSetter(nulls = Nulls.FAIL)
    public ModelValueParams min;

    /**
     * The upper bound (exclusive) of the value (required).
     */
    @JsonSetter(nulls = Nulls.FAIL)
    public ModelValueParams max;

    /**
     * The random seed (optional, default empty).
     */
    ...
    public ModelValue create(Generation generation) {
        return new RandomUniformModelValue(
            min.create(generation),
            max.create(generation),
            generation.seed().salt(seed)
        );
    }
}
