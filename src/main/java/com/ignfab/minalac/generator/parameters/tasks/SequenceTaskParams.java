...
public class SequenceTaskParams extends CompositeTaskParams {
    ...
    public Map<String, TaskParams> flatten(String parentName) {
        ...
        for (TaskParams task : tasks) {
            // Add dependancies between each sequence task
            if (index > 0)
                task.after.add(Integer.toString(index));

            index++;
            populate(result, parentName, Integer.toString(index), task);
        }
        ...
        endTask.after = Set.of(parentName + SEPARATOR + index);
