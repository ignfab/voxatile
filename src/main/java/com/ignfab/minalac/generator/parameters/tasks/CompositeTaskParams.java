...
public abstract class CompositeTaskParams extends ModelTaskParams {
    ...
    protected void populate(Map<String, TaskParams> result, String parentName, String name, TaskParams task) {
        ...
        task.flatten(name).forEach((resultName, resultTask) -> {
            ...
            resultTask.after = resultTask.after.stream()
                .map((after) -> using.contains(after) ? after : makeTaskFullName(parentName, after))
            ...
            // Translate name
            result.put(makeTaskFullName(parentName, resultName), resultTask);
        });
    }

    /**
     * Ensures all subtasks start only when main task dependencies are fulfilled.
     * <p>
     * This will add some {@code after}s on the right tasks so flat schedule will behave exactly like hierarchical schedule described in parameters.
     *
     * @param result Task map to add {@code after}s to
     */
    protected void inheritAfters(Map<String, TaskParams> result) {
        // We could add main dependencies to all subtasks but it is sufficient to add those
        // only to subtasks with no internal dependencies (i.e. having no `after` not in `using`).
    ...
    public Task create(Generation generation) {
        ...
        throw new UnsupportedOperationException("A composite task is not expected to be directly created");
    }
}
