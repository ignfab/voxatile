...
public class MTVoxelTypeParams extends CustomPlaceableParams {
    ...
    public MTVoxelTypeParams(String node) {
        if (node == "")
            throw new IllegalArgumentException("nodeName should not be empty");
        this.node = node;
    }

    @Override
    public Placeable create(VoxelWorld world) {
        if (world instanceof MTVoxelWorld)
            return new MTVoxelType((MTVoxelWorld) world, node, param1, param2);
