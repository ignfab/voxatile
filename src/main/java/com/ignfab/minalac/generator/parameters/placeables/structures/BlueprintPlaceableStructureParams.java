...
public class BlueprintPlaceableStructureParams extends PlaceableStructureParams.Variant {
    ...
    /**
     * List of axes in order of apearance in blueprint (required, must match with blueprint data).
     */
    ...
    Map<Character, PlaceableParams> with;
    ...
    public void validate() {
        ...
        switch (axes.size()) {
            ...
            case 2:
                ...
                if (axes.get(0).direction.equals(axes.get(1).direction))
            ...
            case 3:
                ...
                // Check axes are all different
                if (axes.get(0).direction.equals(axes.get(1).direction)
                    || axes.get(1).direction.equals(axes.get(2).direction)
                    || axes.get(2).direction.equals(axes.get(0).direction))
                    throw new IllegalArgumentException("Same axis used twice.");
                break;
            default:
    ...
    public void apply(VoxelWorld world, PlaceableStructure structure) {
        ...
        // Default hardcoded space char for no voxel
        placeables.put(' ', NoVoxel.INSTANCE);
        // Create placeables for each `with` keys
        with.forEach((key, value) -> {
            if (placeables.containsKey(key))
                throw new IllegalArgumentException("Char '%c' declared twice.".formatted(key));
            placeables.put(key, value.create(world));
        });
        ...
        switch (axes.size()) {
            case 1:
                ...
                process1d(structure, position, axes.get(0), blueprint.data1d());
                break;
            case 2:
                // Process list of strings along first axis for list items and second axis for strings chars
                process2d(structure, position, axes.get(0), axes.get(1), blueprint.data2d());
                break;
            case 3:
                ...
                for (LinkedList<String> data2d : (Iterable<LinkedList<String>>) () -> blueprint.data3d().descendingIterator()) {
                    process2d(structure, position, axes.get(1), axes.get(2), data2d);
    ...
    private void process1d(PlaceableStructure structure, WorldCoords3d position, Axis axis, String data1d) {
        ...
        for (int index = 0, length = data1d.length(); index < length; index++) {
            structure.set(position, getPlaceable(data1d.charAt(index)));
    ...
    private void process2d(PlaceableStructure structure, WorldCoords3d position, Axis listAxis, Axis stringAxis, LinkedList<String> data2d) {
        ...
        // We process lines upside down for a more natural reading
        for (String data1d : (Iterable<String>) () -> data2d.descendingIterator()) {
            process1d(structure, position, stringAxis, data1d);
            position = position.add(direction);
        }
    }

    private record Blueprint(String data1d, LinkedList<String> data2d, LinkedList<LinkedList<String>> data3d) {};

    /**
     * Custom deserializer for blueprint field.
     *
     * This deserializer ensures that we can have only : a string, a list of strings or a list of list of strings.
     * A list mixing strings and list of strings will be rejected.
     */
    private static final class BlueprintDeserializer extends JsonDeserializer<Blueprint> {
        ...
        public Blueprint deserialize(JsonParser jp, DeserializationContext ctxt)
            ...
            if (node.isArray() && node.elements().hasNext()) {
                ...
                // Tree dimensional blueprint (list of lists of strings)
                if (first.isArray()) {
